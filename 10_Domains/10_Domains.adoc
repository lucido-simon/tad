...
I've also defined a Email entity. This entity is here to represent and store emails sent to users (validation emails for example). It is important to note that there is a reference to its corresponding user, via a userId. But, this is only a uuid, meaning that it is not strongly linked to it. Since our email entity is also an aggregate, to enforce consistency, you should be able to modify any child entities of the email aggregate only via the aggregate. This is not what we want here, and we don't need consistency between the two. However, this means that we need to take into account that the user id might not exists anymore.

In the Email entity, I'm also storing the destination email. By doing so, we make sure that we know the destination email at the time the email was sent. A user might change its email, and if we rely on the User entity, we lose this information. Moreover, as discussed above, having a user that exists is not a valid assumption.
